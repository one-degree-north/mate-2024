...
namespace ImGui
{
    ...
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum speed_ is Max(v_speed, minimum_step_at_given_precision).
...
enum ImGuiMouseSource : int
{
    ...
    ImGuiMouseSource_Pen,               // Input is coming from a pressure_/magnetic pen (often used in conjunction with high-sampling rates).
...
struct ImGuiIO
{
    ...
    float       PenPressure;                        // Touch/Pen pressure_ (0.0f to 1.0f, should be >0.0f only when MouseDown[0] == true). Helper storage currently unused by Dear ImGui.
