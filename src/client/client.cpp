...
int main(int argc, char **argv)
{
    ...
    static GstElement *pipeline = gst_parse_launch(
        ...
        "! queue leaky=downstream ! videoconvert ! video/x-raw,format=RGB ! appsink name=gui-sink drop=true sync=false async=false "
        //        "t. ! valve name=image-valve drop=true drop-mode=transform-to-gap ! videocrop name=image-crop bottom=0 ! videorate skip-to-first=true max-closing-segment-duplication-duration=0 ! capsfilter caps-change-mode=immediate name=image-rate-filter caps=video/x-raw,framerate=1/1 ! jpegenc ! multifilesink name=image-sink location=images/image%d.jpeg", //
        "t. ! queue leaky=downstream name=image-queue ! videocrop name=image-crop bottom=0 ! videorate skip-to-first=true max-closing-segment-duplication-duration=0 ! capsfilter caps-change-mode=immediate name=image-rate-filter caps=video/x-raw,framerate=1/1 ! jpegenc ! multifilesink name=image-sink location=images/image%d.jpeg",
        &err);
    if (err)
    {
    ...
    GstElement *imageQueue = gst_bin_get_by_name(GST_BIN(pipeline), "image-queue");
    ...
    union
    {
        struct
        {
            float front, side, up, pitch, roll, yaw, speed;
        };
        uint8_t data[24]{};
    } thrusterData{};
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::Text("Sensor Mode: %s", !bnoDataReceived ? "Waiting For Packet" : bnoIsConfiguring ? "Calibration"
                                                                                                  : "Sensor Data");
        ...
        if (bnoDataReceived)
        {
            ...
            {
                ImGui::SameLine();
                if (ImGui::Button("Switch To Sensor Data"))
                    communication.send({0x03, 0x01});
            ...
            {
                ImGui::SameLine();
                if (ImGui::Button("Switch To Calibration"))
                    communication.send({0x03, 0x00});
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_0, false))
            thrusterData.speed = 0;
        if (ImGui::IsKeyPressed(ImGuiKey_1, false))
            thrusterData.speed = 1;
        if (ImGui::IsKeyPressed(ImGuiKey_2, false))
            thrusterData.speed = 2;
        if (ImGui::IsKeyPressed(ImGuiKey_3, false))
            thrusterData.speed = 3;
        if (ImGui::IsKeyPressed(ImGuiKey_4, false))
            thrusterData.speed = 4;

        thrusterData.speed = std::clamp(thrusterData.speed, 0.0f, 10.0f);
        if (ImGui::SliderFloat("Speed", &thrusterData.speed, 0, 10, "%.2f"))
            controlDataChanged = true;
        ...
        if (ImGui::SliderFloat("Grip", &claw0, -1, 1, "%.2f"))
            gripChanged = true;
        if (gripChanged)
        {
        ...
        ImGui::Text("speed: %f", clawSpeeds[1]);
        ...
        if (ImGui::SliderFloat("Rotation", &claw1, -1, 1, "%.2f"))
            rotChanged = true;
        if (rotChanged)
        {
