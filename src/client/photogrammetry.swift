...
public func run_photogrammetry_session(imagesPath: UnsafePointer<CChar>) {
    ...
    let inputFolderUrl = URL(filePath: String(cString: imagesPath), directoryHint: URL.DirectoryHint.isDirectory)
        ...
        for try await output in session.outputs {
            switch output {
                case .processingComplete:
                    ...
                    let modelAsset = MDLAsset(url: URL(filePath: "out.usdz"))
    ...
    withExtendedLifetime((session, logger)) {
        do {
            try? FileManager.default.removeItem(atPath: "out.usdz")

            let request = PhotogrammetrySession.Request.modelFile(url: URL(filePath: "out.usdz"), detail: .medium)
